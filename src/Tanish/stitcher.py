...
class PanaromaStitcher:
    ...
    def make_panaroma_for_images_in(self, path):
        # Collect and read all images from the given directory
        ...
        # Process each image in sequence to build the panorama
        for i in range(1, len(images)):
            print(f'Stitching image {i}...')
            stitched_image, H = self.stitch_pair(stitched_image, images[i])
            homography_matrix_list.append(H)

        # Crop the black regions from the final panorama
    ...
    def stitch_pair(self, img1, img2):
        ...
        # Place the first image on the warped panorama
        stitched_img = np.zeros_like(warped_img2)
        stitched_img[0:height1, 0:width1] = img1

        # Blend the overlapping regions using alpha blending
    ...
    def crop_black_borders(self, img):
        ...
        # Find contours and get the bounding rectangle of non-black areas
